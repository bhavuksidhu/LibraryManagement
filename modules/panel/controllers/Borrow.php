...
/* ------------------------------------------------------------------------------- */
    ...
    function index()
    {
       ...
       $member= $this->members_model->getMemTypeByID('1');
       $this->data['duedate']=date('Y-m-d', strtotime("+".$member->issue_limit_day ." days"));
        $this->render('borrow/add',$this->data);
   ...
   /* ------------------------------------------------------------------------------- */
    ...
    function getBorrowedBooks($status = NULL) {
       $this->load->library('datatables');
        ...
        $this->datatables
            ->select('borrow_details_id as id, book_title as title, student_name as name, date_borrow as date_borrow, due_date as due_date, date_return as date_return, borrow_status as status,books.isbn as isbn')
            ->from('borrow');
            if ($status === 'returned') {
