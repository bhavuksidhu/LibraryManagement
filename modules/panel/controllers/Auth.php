...
class Auth extends Admin_Controller {
	...
	function create_user()
    {
        ...
        // validate form input
        $this->form_validation->set_rules('student_name', $this->lang->line('create_user_validation_sname_label'), 'required');
        $this->form_validation->set_rules('fathers_name', $this->lang->line('create_user_validation_faname_label'), 'required');
        if($identity_column!=='roll_no')
        {
            $this->form_validation->set_rules('identity',$this->lang->line('create_user_validation_identity_label'),'required|is_unique['.$tables['users'].'.'.$identity_column.']');
            $this->form_validation->set_rules('roll_no', $this->lang->line('create_user_validation_rollno_label'), 'required');
        }
        else
        {
            $this->form_validation->set_rules('roll_no', $this->lang->line('create_user_validation_rollno_label'), 'required|is_unique[' . $tables['users'] . '.roll_no]');
        }
        ...
        $this->form_validation->set_rules('groups[]', $this->lang->line('create_user_validation_member_type_label'), 'required');

        if ($this->form_validation->run() == true)
        {
            $email    = strtolower($this->input->post('email'));
            $roll_no=$this->input->post('roll_no');
            $identity = ($identity_column==='roll_no') ? $roll_no : $this->input->post('identity');
        ...
        else
        {
            ...
            $this->data['student_name'] = array(
                'name'  => 'student_name',
                'id'    => 'student_name',
                'type'  => 'text',
                'value' => $this->form_validation->set_value('student_name'),
            );
            $this->data['fathers_name'] = array(
                'name'  => 'fathers_name',
                'id'    => 'fathers_name',
                'type'  => 'text',
                'value' => $this->form_validation->set_value('fathers_name'),
            );
            ...
            $this->data['roll_no'] = array(
                'name'  => 'roll_no',
                'id'    => 'roll_no',
                'type'  => 'text',
                'value' => $this->form_validation->set_value('roll_no'),
            );
	...
	function edit_user($id)
	{
		...
		// validate form input
		$this->form_validation->set_rules('student_name', $this->lang->line('edit_user_validation_sname_label'), 'required');
		$this->form_validation->set_rules('fathers_name', $this->lang->line('edit_user_validation_faname_label'), 'required');
		$this->form_validation->set_rules('roll_no', $this->lang->line('edit_user_validation_rollno_label'), 'required');
		...
		$this->data['student_name'] = array(
			...
			'id'    => 'student_name',
			'type'  => 'text',
			'value' => $this->form_validation->set_value('student_name', $user->student_name),
		);
		$this->data['fathers_name'] = array(
			'name'  => 'fathers_name',
			'id'    => 'fathers_name',
			'type'  => 'text',
			'value' => $this->form_validation->set_value('fathers_name', $user->fathers_name),
		);
		$this->data['roll_no'] = array(
			'name'  => 'roll_no',
			'id'    => 'roll_no',
			'type'  => 'text',
			'value' => $this->form_validation->set_value('roll_no', $user->roll_no),
		);
		$this->data['phone'] = array(
			'name'  => 'phone',
			'id'    => 'phone',
     ...
     public function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->pname . " (" . $row->roll_no . ")", 'row' => $row);
    ...
    public function deleteOccuModal($id = NULL){

        if (!empty($id)) {
            $this->members_model->deleteClass($id);
            redirect('panel/auth/occupations');
        }
    }
...
function get_suggestions() {
    ...
    if ($rows) {
        foreach ($rows as $row) {
            $pr[] = array('id' => $row->id, 'label' => $row->student_name, 'email' => $row->roll_no, 'name' => $row->student_name);
        }
        $this->lms->send_json($pr);
    } else {
        $this->lms->send_json(array(array('id' => 0, 'label' => lang('no_match_found'), 'value' => $term)));
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                if (!$this->upload->do_upload()) {
                    ...
                    $error = $this->upload->display_errors();
                    $this->session->set_flashdata('error', $error);
                    redirect("panel/auth/create_user");
                }
                ...
                $keys = array('student_name', 'fathers_name', 'roll_no', 'email', 'username', 'phone', 'gender', 'password', 'address','name', 'class_id');
                ...
                $rw = 2;
                $not_savedarr=array();
                $mem=$this->members_model->getAllMemType();
                foreach($final as $csv_pr){
                    $type_id="1";
        foreach($mem as $key){
            if(strtoupper($key->borrowertype)==strtoupper(trim($csv_pr['name']))){
           $type_id=$key->type_id;
           }
        }
                     $identity = trim($csv_pr['roll_no']);
                      $email = trim($csv_pr['email']);
                      $password= (trim($csv_pr['password'])=="")?$email:trim($csv_pr['password']);
                    $additional_data = array(
                 'student_name' => trim($csv_pr['student_name']),
                        'fathers_name' => trim($csv_pr['fathers_name']),
                        'username' => (trim($csv_pr['username'])=="")?$email:trim($csv_pr['username']),
                        'phone' => trim($csv_pr['phone']),
                        'gender' => trim($csv_pr['gender']),
                        'image' => 'no_image.png',
                        'address' => trim($csv_pr['address']),
                        'type_id'=>$type_id,
                        'class_id' => $this->members_model->getClassid(trim($csv_pr['class_id'])),
						'member_unique_id'	=> time() . getmypid(),
            );
            if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $additional_data))
        {
            ...
            $not_savedarr[]=trim($csv_pr['roll_no']);
        }
                }
                if(!empty($not_savedarr)){
                    $this->session->set_flashdata('error', "Error Adding user".json_encode($not_savedarr));
                }
