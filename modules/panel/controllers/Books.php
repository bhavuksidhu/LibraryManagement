...
class Books extends Admin_Controller {
   ...
   /* ------------------------------------------------------------------------------- */
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($m = 0; $m < $s; $m++) {
                ...
                $barcodes[] = array(
                    ...
                    'barcode' => $this->product_barcode($product->isbn, 'code39', $bci_size),
    ...
    function add($id = NULL)
    {
        ...
        $this->form_validation->set_rules('isbn', 'Accession No', 'required|is_unique[books.isbn]');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'bill_date' => $this->input->post('bill_date') ? $this->input->post('bill_date') : date('Y-m-d')
            );
    ...
    function edit($id = NULL)
    {
        ...
        $this->form_validation->set_rules('isbn', 'Accession No', 'required');
        $this->form_validation->set_rules('book_title', 'Book Title', 'required');
        $this->form_validation->set_rules('category_id[]', 'Category', 'required');
        $this->form_validation->set_rules('author_id[]', 'Author', 'required');
        $this->load->library('upload');
    ...
    function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('isbn', 'book_title', 'book_copies', 'book_pub', 'digital_file', 'isbn_13', 'price', 'copyright_year', 'date_receive', 'description', 'categories', 'authors',"book_pages",'bill_date','bill_no');
                ...
                foreach ($final as $csv_pr) {
                    if (!$this->books_model->getBookDetByISBN(trim($csv_pr['isbn']))) {
                        ...
                        $bk_book_pages[] = trim($csv_pr['book_pages']);
                        $bk_bill_date[] = trim($csv_pr['bill_date']);
                        $bk_bill_no[] = trim($csv_pr['bill_no']);
                        $books_add = TRUE;
                    }
                ...
                if ($books_add) {
                   ...
                   if ($bk_authors) {
                    ...
                    $ikeys = array('isbn', 'book_title', 'book_copies', 'book_pub', 'digital_file', 'isbn_13', 'price', 'copyright_year', 'date_receive', 'description', 'categories', 'authors','book_pages','bill_date','bill_no');
               

                    $items = array();
                    foreach (array_map(null, $bk_isbn, $bk_book_title, $bk_book_copies, $bk_book_pub, $bk_digital_file, $bk_isbn_13, $bk_price, $bk_copyright_year, $bk_date_receive, $bk_description, $bk_categories_id, $bk_authors_id,$bk_book_pages,$bk_bill_date,$bk_bill_no) as $ikey => $value) {
