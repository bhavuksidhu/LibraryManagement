...
Class Members_model extends CI_Model
{
    ...
    public function getMemberbyID($id)
    {
        ...
        $q = $this->db->query("SELECT id, student_name as pname,roll_no, member_unique_id, phone, email, class.name as class, borrowertype, section, image
        FROM users
     ...
     public function getMember($term, $limit = 5)
    {
        $q = $this->db->query("SELECT 
            u.id as id, student_name as pname,roll_no, member_unique_id, phone, email, class.name, borrowertype, section, image, 
    ...
    {
        $q = $this->db->query("SELECT 
            u.id as id, student_name as pname,roll_no, member_unique_id, phone, email, class.name, borrowertype, section, image, member_types.type_id, member_types.issue_limit_books, member_types.issue_limit_day,
            GROUP_CONCAT(g.name) groupname 
        FROM 
            users_groups ug   
            INNER JOIN users u ON u.id = ug.user_id   
    ...
    {
        $q = $this->db->get_where('class', array('name' => $name), 1);
        
        if ($q->num_rows() > 0) {
            return $q->result()[0]->class_id;
        }
        return 0;
    }
