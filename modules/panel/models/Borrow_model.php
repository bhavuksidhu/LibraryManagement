...
Class Borrow_model extends CI_Model
{
    ...
    public function GetBorrowDetailsByID($id)
    {
        ...
        $this->db->select('borrow.borrow_id as id, book_title as title, student_name as name, date_borrow as date_borrow, due_date as due_date, date_return as date_return, borrow_status as status, email, phone, price, books.id as book_id, member_types.issue_limit_day');
     ...
     public function GetBorrowDetailsByMemberID($id)
    {
        ...
        $this->db->select('borrow.borrow_id as id, book_title as title, student_name as name, date_borrow as date_borrow, due_date as due_date, date_return as date_return, borrow_status as status, email, phone, price, books.id as book_id, books.isbn as isbn, borrowdetails.borrow_details_id as borrowdetails_tid');
        $this->db->where('users.roll_no', $id);
    ...
    function send_emails($due_ids = NULL, $type = 'email')
    {
        ...
        foreach ($due_ids as $due_id) {
            // echo $due_id;
            $this->db->select('book_title as title, users.email as email, student_name as name, date_borrow as date_borrow, due_date as due_date, users.phone as phone');
            $this->db->join('users', 'borrow.member_id = users.id', 'left');
            $this->db->join('borrowdetails', 'borrow.borrow_id = borrowdetails.borrow_id', 'left');
            $this->db->join('books', ' borrowdetails.book_id =  books.id ', 'left');
